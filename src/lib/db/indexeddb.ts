import { Product, Consumption, CreateProductInput, UpdateProductInput, CreateConsumptionInput, UpdateConsumptionInput } from '@/types/api';
...
export interface OfflineConsumption extends Consumption {
  synced?: boolean;
  updatedAt: Date;
  date: Date;
}
...
export interface SyncOperation {
  ...
  entityId?: number;
  data: CreateProductInput | UpdateProductInput | CreateConsumptionInput | UpdateConsumptionInput | IFormData | { id: number };
  timestamp: Date;
  userId: number;
}

class IndexedDBService {
  private dbName = 'vibeDB';
  private version = 1;
  ...
  async addConsumption(consumption: Omit<OfflineConsumption, 'id'>): Promise<OfflineConsumption> {
    ...
    return new Promise((resolve, reject) => {
      ...
      request.onsuccess = () => {
        resolve({ ...consumption, id: request.result as number });
      };
      request.onerror = () => reject(request.error);
    });
  }
  ...
  async addGoal(goal: Omit<OfflineGoal, 'id'>): Promise<OfflineGoal> {
    ...
    return new Promise((resolve, reject) => {
      ...
      request.onsuccess = () => {
        resolve({ ...goal, id: request.result as number });
      };
      request.onerror = () => reject(request.error);
    });
  }
  ...
  async addSyncOperation(operation: Omit<SyncOperation, 'id'>): Promise<SyncOperation> {
    ...
    return new Promise((resolve, reject) => {
      ...
      request.onsuccess = () => {
        resolve({ ...operation, id: request.result as number });
      };
      request.onerror = () => reject(request.error);
    });
  }
